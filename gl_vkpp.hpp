...
inline void createBufferGL(nvvk::ResourceAllocator& alloc, BufferVkGL& bufGl)
{
  ...
  handleInfo.memory     = info.memory;
  handleInfo.handleType = VK_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_WIN32_BIT_KHR;
  vkGetMemoryWin32HandleKHR(device, &handleInfo, &bufGl.handle);
#else
//  bufGl.fd = device.getMemoryFdKHR({info.memory, vk::ExternalMemoryHandleTypeFlagBits::eOpaqueFd});
  ...
  vkGetMemoryFdKHR(device, &handleInfo, &bufGl.fd);
#endif
...
inline void createTextureGL(nvvk::ResourceAllocator& alloc, Texture2DVkGL& texGl, int format, int minFilter, int magFilter, int wrap)
{
  ...
  handleInfo.memory     = info.memory;
  handleInfo.handleType = VK_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_WIN32_BIT_KHR;
  vkGetMemoryWin32HandleKHR(device, &handleInfo, &texGl.handle);
#else
//  texGl.fd = device.getMemoryFdKHR({info.memory, vk::ExternalMemoryHandleTypeFlagBits::eOpaqueFd});
  VkMemoryGetFdInfoKHR handleInfo{VK_STRUCTURE_TYPE_MEMORY_GET_FD_INFO_KHR};
  handleInfo.memory     = info.memory;
  handleInfo.handleType = VK_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_FD_BIT_KHR;
  vkGetMemoryFdKHR(device, &handleInfo, &texGl.fd);
#endif
